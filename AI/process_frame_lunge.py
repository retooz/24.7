import cv2
import numpy as np
from utils import find_angle, get_landmark_features, draw_dotted_line, draw_text_kor
...
class ProcessFrame:
    def __init__(self, thresholds, fps, frame_size, file_name, connection_code, flip_frame=False):
        ...
        self.out = cv2.VideoWriter(self.file_directory, cv2.VideoWriter_fourcc(
            *'mp4v'), self.fps, self.frame_size)
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                cv2.circle(frame, left_shldr_coord, 3,
                           self.COLORS['yellow'], -1)
                cv2.circle(frame, right_shldr_coord, 3,
            ...
            # 오프셋 각도가 임계값보다 작은 경우
                ...
                draw_dotted_line(
                    frame, hip_coord, start=hip_coord[1]-40, end=hip_coord[1], line_color=self.COLORS['white'])

                # (앞 무릎) 무릎 내각
                front_knee_internal_angle_1 = find_angle(
                    hip_coord, np.array([0, front_knee_coord[1]]), front_knee_coord)
                front_knee_internal_angle_2 = find_angle(
                    front_ankle_coord, np.array([0, front_knee_coord[1]]), front_knee_coord)
                ...
                # (뒷 무릎) 무릎 내각
                back_knee_internal_angle_1 = find_angle(
                    hip_coord, np.array([back_knee_coord[0], 0]), back_knee_coord)
                back_knee_internal_angle_2 = find_angle(
                    back_ankle_coord, np.array([back_knee_coord[0], 0]), back_knee_coord)
                ...
                cv2.line(frame, shldr_coord, elbow_coord,
                         self.COLORS['light_blue'], 2, lineType=self.linetype)
                cv2.line(frame, wrist_coord, elbow_coord,
                         self.COLORS['light_blue'], 2, lineType=self.linetype)
                cv2.line(frame, shldr_coord, hip_coord,
                         self.COLORS['light_blue'], 2, lineType=self.linetype)
                cv2.line(frame, front_knee_coord, hip_coord,
                         self.COLORS['light_blue'], 2,  lineType=self.linetype)
                cv2.line(frame, back_knee_coord, hip_coord,
                         self.COLORS['light_blue'], 2,  lineType=self.linetype)
                cv2.line(frame, front_ankle_coord, front_knee_coord,
                         self.COLORS['light_blue'], 2,  lineType=self.linetype)
                cv2.line(frame, front_foot_coord, front_ankle_coord,
                         self.COLORS['light_blue'], 2,  lineType=self.linetype)
                cv2.line(frame, back_ankle_coord, back_knee_coord,
                         self.COLORS['light_blue'], 2,  lineType=self.linetype)
                cv2.line(frame, back_foot_coord, back_ankle_coord,
                         self.COLORS['light_blue'], 2,  lineType=self.linetype)

                # 랜드마크 점 시각화
                cv2.circle(frame, shldr_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, elbow_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, wrist_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, hip_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, front_knee_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, back_knee_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, front_ankle_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, back_ankle_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, front_foot_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, back_foot_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                ...
                if current_state == 's1':
                    if self.state_tracker['rec'] == True:
                        ...
                        self.out = cv2.VideoWriter(self.file_directory, cv2.VideoWriter_fourcc(
                            *'mp4v'), self.fps//2, self.frame_size)
                        self.state_tracker['SEP_LIST'].append(
                            self.file_directory)
                ...
                if self.flip_frame:
                    ...
                    front_knee_text_coord_x = frame_width - \
                        front_knee_coord[0] + 15
                    back_knee_text_coord_x = frame_width - \
                        back_knee_coord[0] + 10

                # 프레임에 엉덩이, 무릎, 발목의  수직 각도를 나타내는 텍스트 표시
                cv2.putText(frame, str(int(hip_internal_angle)), (hip_text_coord_x,
                            hip_coord[1]), self.font, 0.4, self.COLORS['green'], 1, lineType=cv2.LINE_AA)
                cv2.putText(frame, str(int(front_knee_internal_angle)), (front_knee_text_coord_x,
                            front_knee_coord[1]+10), self.font, 0.4, self.COLORS['light_green'], 1, lineType=cv2.LINE_AA)
                cv2.putText(frame, str(int(back_knee_internal_angle)), (back_knee_text_coord_x,
                            back_knee_coord[1]+10), self.font, 0.4, self.COLORS['light_green'], 1, lineType=cv2.LINE_AA)
