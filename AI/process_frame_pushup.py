...
import cv2
import numpy as np
from utils import find_angle, get_landmark_features, draw_text_kor, draw_dotted_line, hip_find_angle
# utils 사용 이유
        ...
        if keypoints.pose_landmarks:
            ...
            # 오프셋 각도가 임계값보다 작은 경우
                ...
                shldr_vertical_angle = find_angle(
                    hip_coord, np.array([shldr_coord[0], 0]), shldr_coord)
                ...
                elbow_vertical_angle = find_angle(
                    shldr_coord, np.array([elbow_coord[0], 0]), elbow_coord)
                ...
                hip_vertical_angle = find_angle(shldr_coord, np.array(
                    [hip_coord[0], 0]), hip_coord) + find_angle(knee_coord, np.array([hip_coord[0], 0]), hip_coord)
                cv2.ellipse(frame, hip_coord, (30, 30),
                ...
                draw_dotted_line(
                    frame, shldr_coord, start=shldr_coord[1]-40, end=shldr_coord[1], line_color=self.COLORS['white'])
                draw_dotted_line(
                    frame, elbow_coord, start=elbow_coord[1]-40, end=elbow_coord[1], line_color=self.COLORS['white'])
                ...
                cv2.putText(frame, str(int(shldr_vertical_angle)), (shldr_text_coord_x,
                            shldr_coord[1]), self.font, 0.4, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
                cv2.putText(frame, str(int(elbow_vertical_angle)), (elbow_text_coord_x,
                            elbow_coord[1]+10), self.font, 0.4, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
