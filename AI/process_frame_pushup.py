...
class ProcessFrame:
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # 왼쪽 팔길이가 오른쪽 팔길이보다 클 경우, 왼쪽 좌표 변수 할당
            ...
            hip_vertical_angle = find_angle(shldr_coord, knee_coord, hip_coord)
            cv2.ellipse(frame, hip_coord, (30, 30), 
                        angle = 0, startAngle = 0, endAngle = 180+multiplier*hip_vertical_angle, 
                        color = self.COLORS['white'], thickness = 2, lineType = self.linetype)
            ...
            if current_state == 's1':
                if self.state_tracker['rec'] == True:
                    ...
                    self.ex_count += 1
                    self.file_directory = f'C:\\Users\\gjaischool\\Desktop\\test\\output{self.ex_count}.mp4'
                    self.out = cv2.VideoWriter(self.file_directory, cv2.VideoWriter_fourcc(*'mp4v'), self.fps//3, self.frame_size)
                
                if len(self.state_tracker['SHLDR_ANGLE_LIST']) > 3:
                    max_shldr = max(self.state_tracker['SHLDR_ANGLE_LIST'])
                    score = self._get_score(
                        self.state_tracker['ANGLE_LIST'][self.state_tracker['SHLDR_ANGLE_LIST'].index(max_shldr)])
                    self.state_tracker['SCORE_LIST'].append(score)
                    
                # 변수 초기화
                self.state_tracker['ANGLE_LIST'] = []
                self.state_tracker['SHLDR_ANGLE_LIST'] = []
            ...
            # 현재 상태가 s1이 아닌 경우
                ...
                self.state_tracker['SHLDR_ANGLE_LIST'].append(hip_vertical_angle)
            ...
            cv2.putText(frame, str(int(shldr_vertical_angle)), (shldr_text_coord_x, shldr_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
            cv2.putText(frame, str(int(elbow_vertical_angle)), (elbow_text_coord_x, elbow_coord[1]+10), self.font, 0.6, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
            cv2.putText(frame, str(int(hip_vertical_angle)), (hip_text_coord_x, hip_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
