...
class ProcessFrame:
    ...
    def _get_score(self, angle):
        ...
        if 76 <= knee_angle <= 90:
            score += 100
        elif 74 <= knee_angle <= 91:
            score += 90
        elif 72 <= knee_angle <= 92:
            score += 80
        elif 70 <= knee_angle <= 93:
            score += 70
        elif 68 <= knee_angle <= 94:
            score += 60
        elif 66 <= knee_angle <= 95:
            score += 50
        elif 64 <= knee_angle <= 96:
            score += 40
        elif 62 <= knee_angle <= 97:
            score += 30

        if 20 <= ankle_angle <= 27:
            score += 100
        elif 19 <= ankle_angle <= 28:
            score += 90
        elif 18 <= ankle_angle <= 29:
            score += 80
        elif 17 <= ankle_angle <= 30:
            score += 70
        elif 16 <= ankle_angle <= 31:
            score += 60
        elif 15 <= ankle_angle <= 32:
            score += 50
        elif 14 <= ankle_angle <= 33:
            score += 40
        elif 13 <= ankle_angle <= 34:
            score += 30
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # 오프셋 각도가 임계값보다 작은 경우
                ...
                draw_dotted_line(
                    frame, knee_coord, start=knee_coord[1]-40, end=knee_coord[1], line_color=self.COLORS['white'])
                ...
                draw_dotted_line(
                    frame, ankle_coord, start=ankle_coord[1]-40, end=ankle_coord[1], line_color=self.COLORS['white'])
                ...
                cv2.circle(frame, hip_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, knee_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, ankle_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)
                cv2.circle(frame, foot_coord, 3,
                           self.COLORS['yellow'], -1,  lineType=self.linetype)

                # 현재 상태를 가져오고, 상태 시퀀스 업데이트.
                hip_ankle_knee_angle = find_angle(
                    hip_coord, ankle_coord, knee_coord)
                ...
                cv2.putText(frame, str(int(hip_vertical_angle)), (hip_text_coord_x,
                            hip_coord[1]), self.font, 0.4, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
                cv2.putText(frame, str(int(knee_vertical_angle)), (knee_text_coord_x,
                            knee_coord[1]+10), self.font, 0.4, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
                cv2.putText(frame, str(int(ankle_vertical_angle)), (ankle_text_coord_x,
                            ankle_coord[1]), self.font, 0.4, self.COLORS['light_green'], 2, lineType=cv2.LINE_4)
