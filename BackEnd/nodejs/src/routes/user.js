    ...
    try {
        ...
        if (result.affectedRows > 0) {
            ...
            if (userResult.length > 0) {
                ...
                req.session.user.email = userResult[0].email
                req.session.user.code = userResult[0].user_code
                res.send({ result: 1 })
            }
        } else {
            res.send({ result: 0 })
...
router.post('/sendTrainer', upLoadVideo, async (req, res) => {
    try {
        ...
        const videoPath = path.join('/uploads','video',`${connectionCode}`)
        const setVideoUrl = await userService.setVideoUrl(path.join(`${videoPath}`,`${splitFileName}`), connectionCode);
        if (checkAi == 'Ai') {
...
router.post('/getFeedback', async (req, res) => {
    try {
        ...
        if (result.length > 0) {
            ...
            if(accuracyData[0].accuracy !== null){
            ...
            }else if(accuracyData[0].accuracy == null){
                res.json({ result: result, trainer: trainer, accuracyData: accuracyData })
            }
        } else {
